 ...
 */
function signPayload(data, privateKeyBase64url) {
    const privateKeyDer = Buffer.from(privateKeyBase64url.replace(/-/g, "+").replace(/_/g, "/"), "base64");
...
export class GatewayOperatorClient extends EventEmitter {
        ...
        try {
            ...
            if (msg.type === "res" && msg.id) {
                ...
                if (msg.result && typeof msg.result === "object" && "deviceToken" in msg.result) {
                    this.deviceToken = msg.result.deviceToken;
    ...
    handleApprovalRequest(payload) {
        ...
        const { approvalId, sessionKey, command } = payload;
        if (!approvalId) {
            return;
        }
